...
os.chdir("C://Users//nadja//Documents//PLIC Segmentation//PLIC_pytorch//Data_npz//")
data = np.load("classification_data.npz",allow_pickle=True)
X_train = data["X_train"]
X_train_new = np.moveaxis(X_train, 3,1)
X_test = data["X_test"]
X_new = np.concatenate((X_train, X_test),axis=0)

X_test_new = np.moveaxis(X_test, 3,1)
data_patches = np.load("Labels_patches.npz", allow_pickle=True)
Y_patch = data_patches["Y_patch"] #all patches of Labels available 10854
Y_patch_new = np.moveaxis(Y_patch, 3,1)
indices = np.load("indices.npz")
indices = indices["indices"]
data = torch.cat([torch.tensor(X_train_new),torch.tensor(Y_patch_new[:-1416])],axis=1)
data_val = torch.cat([torch.tensor(X_test_new), torch.tensor(Y_patch_new[-1416:])], axis=1)
...
class Segmentation_of_PLIC:
    ...
    def create_sag_cor_patches_test(self, X_new,  indices):
        ...
        for i in [50]:
            thalamus_train = list(np.ones(85)*i)
            sag = preprocessing_sagittal_test(X_new, indices, thalamus_train)
            cor = preprocessing_coronal_test(X_new, indices,thalamus_train)
            Y_train_sag.append(np.moveaxis(sag[1][:,:,:,:1], 3, 1))      
            X_train_sag.append(np.moveaxis(sag[0], 3, 1))
...
mynet.create_sag_cor_patches( X_new, Y_patch, indices)
...
weight_path = "C:/Users/nadja/Documents/PLIC Segmentation/PLIC_pytorch/"
...
def apply_NW(weight_path, mynet, indices):
    Sag = []
    Cor = []
    Combi=[]
    Ax=[]
    ...
    for j in range(len(indices[85:97])):
        ...
        return(Combi)
output=apply_NW(weight_path, mynet,indices)