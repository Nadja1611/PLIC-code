# -*- coding: utf-8 -*-
"""
Created on Mon Aug 28 10:27:48 2023
...
'''standardizes the data and makes patches out of original images'''
def preprocessing(X):
    X = X[:,65:129,65:129]
    liste=[]
...
def create_boxes_cor(X,levels,indices):
    ...
    for i in range(len(levels)):
        ...
        Matrix[:,int(level)-12:int(level)+12,:]=1
        image = X[:,int(level)-32:int(level)+32,:]
        Images.append(image)  
    Images=np.asarray(Images)
    return Matrix,Images

def create_boxes_cor_test(X,levels,indices):
    Images=[]
    image = X[:,int(levels)-32:int(levels)+32,:]
    Images.append(image)  
    Images=np.asarray(Images)
    return Images
def create_boxes_sag_test(X,levels,indices):
    Images=[]
    image = X[:,int(levels)-32:int(levels)+32,:]
    Images.append(image)  
    Images=np.asarray(Images)
...
'''put back axial results to original sliced images of size 192x192'''
def reconstruct_axial(X, indices1, indices2):
#liste.append(T1[:indices[0]])
        indices= indices1- indices2
        X_org = np.zeros((indices,192,192))
        liste = np.zeros((indices, 192,192))
        M = liste
        M=M.astype("float32")
        print(indices1)
        print(indices2)
        ...
        M[:, 65:129,65:129]=X[indices2:indices1,:,:]

        return M
    
'''put back sagittal results to original sliced images of size 192x192'''
def reconstruct_sag(X, indices,levels):
#liste.append(T1[:indices[0]])

        X_org = np.zeros((indices,192,192))
        liste = np.zeros((indices, 192,192))
            
        M = np.zeros_like(np.moveaxis(np.asarray(liste),2,0))
        M=M.astype("float32")


        M[65:129,int(levels)-32:int(levels)+32,65:129]=X[:,:,:]
        M = np.moveaxis(M, 1, 0)
        M = np.moveaxis(M , 2,1)
        return M
...
'''put back sagittal results to original sliced images of size 192x192'''
def reconstruct_cor(X, indices,levels):
#liste.append(T1[:indices[0]])

        X_org = np.zeros((indices,192,192))
        liste = np.zeros((indices, 192,192))
            
        M = np.zeros_like(np.moveaxis(np.asarray(liste),0,1))
        M=M.astype("float32")
...
#         X_org = np.zeros((indices[i],192,192))
#         P = X_org
#         P=np.asarray(P)
#         liste.append(P)
...
def preprocessing_coronal_test(X,indices, thalamus_val):
    X_val = np.moveaxis(X[indices],0,1)
    X_res_val = create_boxes_cor_test(X_val,thalamus_val,indices)[1]
    X_res_val = preprocessing_no_patching(X_res_val)
    X_val=np.concatenate(X_res_val,axis=0)
 #   Y_train = np.expand_dims(Y_train, axis=-1)
 #   Y_val = np.expand_dims(Y_val, axis=-1)
...
def preprocessing_cor_test(test,indices_test2,thalamus):
    test = test.squeeze(1)
    X_train = np.moveaxis(np.asarray(test),0,1)
    X_res = create_boxes_sag_test(X_train,thalamus,indices_test2)
    X_res = preprocessing_no_patching(X_res)
    X_train=np.concatenate(X_res,axis=0)
...
def preprocessing_sag_test(test,indices_test2,thalamus):
    test = test.squeeze(1)
    X_train = np.moveaxis(np.asarray(test),2,0)
    X_res = create_boxes_sag_test(X_train,thalamus,indices_test2)
    X_res = preprocessing_no_patching(X_res)
    X_train=np.concatenate(X_res,axis=0)
    X_train = np.moveaxis(X_train, 3,1)
...
def make_patient_list(path):
    ''' this function reads in the data from a specified path and creates a list'''
    data = np.load(path+"/Training_Babies.npz",allow_pickle=True)
    patients= data["T1"]
...
def dice_loss(y_true, y_pred):
 ...
 # y_pred = tf.sigmoid(y_pred[:,:,:,0])
    numerator = 2 * torch.sum(y_true[:,:,:,0] * y_pred[:,:,:,0])
    denominator = torch.sum(y_true[:,:,:,0] + y_pred[:,:,:,0]+ epsilon)
...
#liste.append(T1[:indices[0]])
   ...
   # Matrix = np.asarray(Matrix)    
    for j in range(len(levels)):
        index1 = int(j*64)
        index2 = int(j*64 +64)
        Matrix[j][65:129,int(levels[j])-32:int(levels[j])+32,65:129]=X[index1:index2,:,:]
